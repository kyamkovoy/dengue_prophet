...
for j in range(1,11):
    ...
    for prov in province_codes:
        for k in range(0, 11 - j):
            ...
            file_name = '/prov_' + str(prov) + '_monthly_' + str(j) + '/prov_' + str(prov) + '_for_' + str(year) + '_monthly_' + str(j) + '.pkl'
    ...
    cv_df.to_csv(folder_name + '/' + 'monthly_errors_' + str(j) + '.csv', index=False)
